	...
	for tr in range(i, len(trs) - 1):
		...
		if validRow == True:
			...
			if len( data) > 1:
				...
				data = bs_Read(player_url).find('ul',{'class':'player-metadata floatleft'})
				lis = data.findAll('li')
				...
				if(position != "P"):
					...
					averageWar = warSum/cumsumYears
					averageGame = gamesSum/cumsumYears
					averagePA_IN = paSum/cumsumYears
					averageOPS_FIP = ops/percentageYears
					averageBABIP = babip/percentageYears
					rate_K = rate_K/percentageYears
					rate_BB = rate_BB/percentageYears
					...
					averageWar = warSum/cumsumYears
					averageGame = gamesSum/cumsumYears
					averagePA_IN = inningsSum/cumsumYears
					averageOPS_FIP = fip/percentageYears
					averageBABIP = babip/percentageYears
					rate_K = rate_K/percentageYears
					rate_BB = rate_BB/percentageYears
